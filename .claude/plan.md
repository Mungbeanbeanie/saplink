...
- [x] `esp32/Saplink/lib/saplink_common/signal_conditioning.h/.cpp` — `class SignalConditioner`: `float update(float raw_mv)` runs a small fixed-window median filter (odd window, 5 samples), then subtracts a slow-adapting baseline (`baseline += (filtered - baseline) * alpha`). Exposes `float baseline() const` for serial debug logging. Meant to be called from `combo_main.cpp`'s sense loop, in place of its raw `readMv()` passthrough. (60Hz notch dropped: sample rate is 10Hz, Nyquist=5Hz, notch is meaningless below Nyquist and was never viable at this rate; median filter + adaptive baseline only. Revisit only if sample rate is raised well above 120Hz.) (GAINED `deviation()`, `sigma()` and `warm()` when wired to a real electrode, plus fixes for two cold-start bugs that only appear against a real standing offset. (1) `baseline_` started at `0.0f`; a real electrode sits tens of mV off zero, so every boot opened with a phantom decay lasting one tau, shaped exactly like the event the detector hunts — now seeded from the first sample. (2) Nothing ever computed `PeakDetector`'s `baseline_sigma`; the test passed a hardcoded `1.0f`. `sigma()` now derives it from a MAD estimate, floored at one ADS1115 LSB so a quiet electrode cannot drive the threshold to zero. That estimator had the *same* cold start — creeping up from `mad_ = 0` at alpha 0.001 made every batch on hardware report SPIKE — so it warms via a running mean and `warm()` gates detection off for the first 100 samples. Under-estimated sigma does not mis-fire, it fires on everything, so the honest pre-convergence state is "don't know". Six native tests now cover these; three would fail without the fixes.)
...
- [ ] `esp32/Saplink/src/combo_main.cpp` — **primary entrypoint, partially built.** Landed and compiling today: WiFi join with retry (`wifiUp()`), the `readMv()` seam (synthetic wander+spike, `SRC="sim"`), 32-sample batches at 10Hz, and HTTPS POST of Contract A to `/api/readings`. Outstanding: swap `readMv()` for a real ADS1115 differential read; wire in Phase 2's `SignalConditioner`/`PeakDetector`/`RecordedSignalPlayer`; and add the poll+actuate+ack half (drives the relay/pump/LED per the Actuate design, using whatever Contract B alert routes Phase 5 builds), then acking so the same alert can't fire twice. **Blocked** on Phase 5's alert control plane not existing yet. (SENSE HALF NOW DONE and verified on hardware end to end: `src=ads1115`, `POST 200`, real electrode + soil probe. `readMv()` reads A2-A3 via Adafruit ADS1X15 at GAIN_FOUR, falling back to the synthetic trace if `ads.begin()` fails; `SignalConditioner` + `PeakDetector` are in the sample loop; `mv[]` carries `deviation()`. `RecordedSignalPlayer` is still NOT wired — it remains the only Phase 2 primitive combo does not call. Actuate half still blocked as stated. Two cold-start bugs in `SignalConditioner` had to be fixed first, both of which only show up against a real electrode's standing offset — see its checklist item.)
