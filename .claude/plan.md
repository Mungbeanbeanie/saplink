...
**Outdoor temperature (built):** New `app/backend/weather.py` fetches Blacksburg, VA's current temperature from Open-Meteo (free, no API key) on a background refresh thread (`WEATHER_REFRESH_SECONDS`, mirrors `NEWS_REFRESH_SECONDS`), cached in memory (no DB table -- one current value, not a history). New `GET /api/weather` (public). Frontend's `src/lib/weather.js` (`useWeather()`) feeds a second cell in the Conditions card, labeled "Outdoor temp (Blacksburg)" -- explicitly not presented as a plant/hardware reading. **Hardcoded to one fixed location, no dynamic per-router location** -- user-scoped for this pass; `Dashboard.jsx` no longer has a `roster.js` fixture with per-router coordinates anyway (device tabs come from `/api/health`'s real device list, see `architecture.md`), so per-router weather would need a real lat/lon source that doesn't exist yet.
