...
**Outdoor temperature (backend built, NOT currently displayed anywhere):** `app/backend/weather.py` fetches Blacksburg, VA's current temperature from Open-Meteo (free, no API key) on a background refresh thread (`WEATHER_REFRESH_SECONDS`, mirrors `NEWS_REFRESH_SECONDS`), cached in memory (no DB table -- one current value, not a history). `GET /api/weather` (public) and `src/lib/weather.js` (`useWeather()`) both still exist and work, but the Conditions card that displayed this was removed (see Moisture note above) and `Dashboard.jsx` no longer imports `useWeather` — a deliberate, temporary gap: user wants it folded into "The site" card in a later pass, not dropped as a feature. **Hardcoded to one fixed location, no dynamic per-router location** -- user-scoped for this pass; `Dashboard.jsx` has no per-router coordinate source anyway (device tabs come from `/api/health`'s real device list, see `architecture.md`), so per-router weather would need a real lat/lon source that doesn't exist yet.
