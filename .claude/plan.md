...
- [x] `GET /api/weather` — Blacksburg's current outdoor temperature via `app/backend/weather.py` (Open-Meteo, no API key, background-refreshed, in-memory cache only). Fixed location, no per-router dynamic lookup. Feeds Phase 6's Conditions card.
...
**Moisture (built, moved into "The site" card):** The standalone Conditions card was removed. `Dashboard.jsx` tracks `soil_mv` from the latest sample and shows it as a small raw-mV tag in "The site" card's header ("Soil moisture: N mV (raw)") — still not a displayed `%`, matching `combo_main.cpp`'s "map both at once, or neither" reasoning. A real per-probe two-point calibration DOES now exist (`esp32/Saplink/src/soil_main.cpp`'s measured `kDryMv`/`kWetMv`, hardcoded into `Dashboard.jsx` as `SOIL_CALIBRATION`, keyed by device: `sense-1` dry 2154mV/wet 865mV, `sense-2` dry 2175mV/wet 910mV) — it's used ONLY to compute a 0-1 wetness fraction driving a blue radial-gradient glow centered on the site graph (bigger/more opaque = wetter; higher raw mV is drier, so the fraction inverts the raw value), never rendered as a number. This needed a companion fix: the readings poll (`tick()`) didn't pass `&device=` even though the backend supports it, so `soilMv` could reflect whichever device's batch arrived last — fixed by keying that effect on `device` and resetting `lastId`/`samples`/`soilMv` on tab switch.
...
**Status over time (built):** `Dashboard.jsx`'s "Status over time" card was a "not live yet" placeholder (no per-hour history endpoint existed). New backend `GET /api/status_history?device=&hours=48` (Phase 5) buckets `batch` rows by hour from `recv_ts`, filling every hour in the window even with zero batches. New `src/lib/statusHistory.js` (`useStatusHistory(device,hours)`) polls it; the card renders one block per hour, colored client-side (0 batches = gray/offline, any `event=='spike'` that hour = orange, else green/reporting), tracking whichever device tab is selected. (The readings chart/history poll has SINCE also become per-device filtered — see the Moisture note below — so this is no longer the only per-device-correct data source on the page.)

**Outdoor temperature (built, folded into "The site" card):** `app/backend/weather.py` fetches Blacksburg, VA's current temperature from Open-Meteo (free, no API key) on a background refresh thread (`WEATHER_REFRESH_SECONDS`, mirrors `NEWS_REFRESH_SECONDS`), cached in memory (no DB table -- one current value, not a history). `GET /api/weather` (public) and `src/lib/weather.js` (`useWeather()`) now both feed `Dashboard.jsx`'s "The site" card: a grey vertical thermometer bar to the left of the site-map graph, filling bottom-up with a pink→purple→orange→red gradient (`TEMP_MIN_F`/`TEMP_MAX_F` = 20/100°F, a tuned display heuristic like `FULL_GLOW_MV`), with the current temperature labeled at the fill's edge. The local dev simulator (`server/index.js`) gained a matching `/api/weather` mock route so this renders in `npm run dev` too. **Hardcoded to one fixed location, no dynamic per-router location** -- user-scoped for this pass; `Dashboard.jsx` has no per-router coordinate source anyway (device tabs come from `/api/health`'s real device list, see `architecture.md`), so per-router weather would need a real lat/lon source that doesn't exist yet.

**Humidity/Light — OPEN DECISION, still not built:** No sensor exists for either today (outside the current BOM). Humidity could follow the same fixed-Blacksburg shortcut as temperature above, but wasn't in scope for this pass. Light level (lux) has no weather-API equivalent at all — that one needs an actual light sensor regardless of any API integration.
