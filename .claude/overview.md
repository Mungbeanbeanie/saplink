...
The cloud hub is live today on two Vultr instances: `saplink-api` (Caddy terminating TLS, reverse-proxying to a FastAPI app backed by SQLite) and `saplink-web` (Caddy serving a static dashboard, no build step). An ESP32 running `sensor_main.cpp` joins WiFi, batches voltage samples, and POSTs them over HTTPS to `saplink-api`'s `/ingest` endpoint, authenticated with a bearer token; the dashboard on `saplink-web` polls `saplink-api`'s `/samples` and `/events` endpoints directly from the browser (CORS-allowed, no auth needed for reads). This sense→cloud half of the loop works today, with synthetic data (a slow wander plus a periodic spike) standing in for the ADS1115 until it's physically wired — swapping in a real read is a one-function change, by design ("the seam," see `esp32/Saplink/CLAUDE.md`).
...
- **Interface Schema:** The frozen wire format — one POST covers a batch of samples, not a single alert: `device` (str, ≤32 chars), `seq` (int ≥0, monotonic per boot — gaps mean dropped batches), `t_ms` (int ≥0, `millis()` at the *first* sample in the batch), `period_ms` (int, 1–60000), `baseline_mv` (float, −5000..5000mV), `event` (`null` or `"spike"`), `src` (`"sim"` or `"ads1115"` — how live data is told apart from synthetic on stage), `mv` (array of ≤256 floats, each −5000..5000mV). Canonical doc: `esp32/Saplink/CLAUDE.md`'s "frozen wire format" section — update there first to avoid the same fields drifting across three descriptions.
