...
- **Edge Classifier:** Not yet built. The wire schema reserves an `event` field (`null` or `"spike"`) for this; `sensor_main.cpp` always sends `event:null` today. Planned implementation (Phase 2's `PeakDetector` logic, 3σ-threshold + ≥30% rebound-ratio, from PlantLeaf's method) now runs backend-side, in `saplink-api`'s `main.py`, classifying the live Contract A stream per-device — not on the ESP32. The on-device `PeakDetector` C++ class is retained only for `RecordedSignalPlayer`'s local fallback-trigger path (see Actuate Subsystem's Manual Replay Path), which is designed to work with no cloud round-trip.
